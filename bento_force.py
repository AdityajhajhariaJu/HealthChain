...
'''
        content = parts[0] + new_layout + end_str + sub_parts[1]
        
        with open('src/features/dashboard/CaseDashboard.tsx', 'w', encoding='utf-8') as f:
            f.write(content)
        print('Successfully replaced Bento layout in CaseDashboard.tsx!')
    else:
        print('Error: Could not find end_str')
else:
    print('Error: Could not find split_str')
