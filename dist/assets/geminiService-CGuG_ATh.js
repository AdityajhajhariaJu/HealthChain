import{g as C,b as x}from"./index-DNh-IHNl.js";import"./framer-motion-CDW5Q6YF.js";import"./react-vendor-DURbyAIq.js";import"./pdf-tools-BU1OiuD_.js";import"./supabase-BddhP1o-.js";let S=null,b=null;function T(){var i,t;let n=[];const a=C(),e=x(),c=`${a==null?void 0:a.id}-${a==null?void 0:a.updatedAt}-${e==null?void 0:e.id}-${e==null?void 0:e.updatedAt}`;if(S&&b===c)return S;let r=`PATIENT PROFILE:
...
`),a.medications&&a.medications.length>0&&(r+=`- Meds: ${a.medications.map(o=>o.name).join(", ")}
`),a.familyHistory&&a.familyHistory.length>0&&(r+=`- Family Hx: ${a.familyHistory.join(", ")}
`),r!==`PATIENT PROFILE:
`&&n.push(r);const d=Object.entries(a.vitals.latestLabValues);if(d.length>0){let o=`LABS:
`;d.slice(0,8).forEach(([h,p])=>{o+=`- ${h}: ${p.value} ${p.unit} (${p.status})
`}),n.push(o)}if(e){let o=`ACTIVE CASE:
`;o+=`- ${e.title}
`,(i=e.intakeData)!=null&&i.chiefComplaint&&(o+=`- Concern: ${e.intakeData.chiefComplaint.substring(0,200)}
`),(t=e.report)!=null&&t.executiveSummary&&(o+=`- Synthesis: ${e.report.executiveSummary.substring(0,200)}
`);const h=(e.medicalRecords||[]).slice(0,3);h.length&&(o+=`- Evidence: ${h.map(p=>`${p.filename} (${p.keyFindings||"On file"})`).join("; ")}
`),n.push(o)}const s=a.timeline.filter(o=>["diagnosis","mdt_report","lab_report"].includes(o.type));if(s.length>0){let o=`HISTORY:
`;s.slice(0,3).forEach(h=>{o+=`- ${new Date(h.date).toLocaleDateString()}: ${h.title}
`}),n.push(o)}if(n.length===0)return S="",b=c,"";let l=`

=== PATIENT CONTEXT ===
${n.join(`
`)}========================
`;return l.length>1500&&(l=l.substring(0,1497)+`...
`),S=l,b=c,l}const u="/api/gemini",y=async(n,a={},e=6e4,c=2)=>{if(typeof navigator<"u"&&!navigator.onLine)throw new Error("Offline");try{const i=Date.now();let t=[];try{t=JSON.parse(localStorage.getItem("hc_api_logs")||"[]")}catch{t=[]}if(t=t.filter(o=>i-o<36e5),t.length>=20)throw new Error("Rate limit exceeded. Please try again later.");t.push(i),localStorage.setItem("hc_api_logs",JSON.stringify(t))}catch(s){if(s.message.includes("Rate limit exceeded"))throw s}let r;const d={...a,headers:{...a.headers}};for(let s=0;s<=c;s++){const l=new AbortController,i=setTimeout(()=>l.abort(),e);try{const t=await fetch(n,{...d,signal:l.signal});if(clearTimeout(i),t.status===429&&s<c){await new Promise(o=>setTimeout(o,1e3*(s+1)));continue}if(!t.ok&&t.status>=500&&s<c){await new Promise(o=>setTimeout(o,1e3*Math.pow(2,s)));continue}return t}catch(t){if(clearTimeout(i),r=t,(t.name==="AbortError"||t.name==="TypeError")&&s<c){await new Promise(o=>setTimeout(o,1e3*Math.pow(2,s)));continue}if(s===c)throw t.name==="AbortError"?new Error("Timeout"):t}}throw r},v=`You are HealthChain's clinical investigation AI.
...
Do NOT include ANALYSIS_COMPLETE until you are ready to conclude.`;async function j(n){var l,i;const c=(((l=n[0])==null?void 0:l.role)==="model"?n.slice(1):n).slice(-12).map(t=>{let o=t.content;return t.role==="analysis"&&(o=`ANALYSIS_COMPLETE
\`\`\`json
${JSON.stringify(t.content)}
\`\`\``),{role:t.role==="user"?"user":"model",parts:[{text:o}]}}),r=T(),s={systemInstruction:{role:"system",parts:[{text:v+r}]},contents:c};try{const t=await y(u,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(s)});if(!t.ok)throw new Error(`API Error: ${t.status}`);const o=await t.json();return(i=o.candidates)!=null&&i[0]?o.candidates[0].content.parts[0].text:"Could you tell me a bit more about that?"}catch{return"Connection issue. Please try again."}}const P=`You are a clinical pharmacology AI.
...
Please strictly evaluate for interactions.`);const c={systemInstruction:{role:"system",parts:[{text:P}]},contents:[{role:"user",parts:[{text:e}]}],generationConfig:{responseMimeType:"application/json"}};try{const d=await y(u,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(c)});if(!d.ok)throw new Error(`API Error: ${d.status}`);const s=await d.json();if((r=s.candidates)!=null&&r[0]){const i=s.candidates[0].content.parts[0].text.replace(/```json/g,"").replace(/```/g,"").trim();return JSON.parse(i)}throw new Error("No candidate returned")}catch{return null}}const O=`You are Ava, HealthChain's "Medical Chief of Staff" and Personal Health Assistant.
...
`;async function R(n){var d;const a=n.slice(-12).map(s=>({role:s.role==="user"?"user":"model",parts:[{text:s.content}]})),e=T(),r={systemInstruction:{role:"system",parts:[{text:O+e}]},contents:a};try{const s=await y(u,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(r)});if(!s.ok)throw new Error(`API Error: ${s.status}`);const l=await s.json();return(d=l.candidates)!=null&&d[0]?l.candidates[0].content.parts[0].text:"I'm here for you. Could you tell me a bit more?"}catch{return"I'm having a little trouble connecting right now, but I'm still here for you."}}const A=`You are HealthChain's "Clinical Lab Interpreter", a highly advanced medical AI capable of reading lab reports, blood work, MRIs, and prescriptions.
...
You are part of a Collaborative Board alongside: ${d}.
...
}`}]},contents:[{role:"user",parts:[{text:"Run the Board Conference based on the provided specialist data."}]}],generationConfig:{responseMimeType:"application/json"}};try{const i=await y(u,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(s)});if(!i.ok)throw new Error(`API Error: ${i.status}`);const t=await i.json();if((l=t.candidates)!=null&&l[0]){const h=t.candidates[0].content.parts[0].text.replace(/```json/g,"").replace(/```/g,"").trim();return JSON.parse(h)}}catch{return{corroborations:[],contentions:[],followUpQuestions:[],debateSummary:"Board consensus failed due to an error."}}return null}async function H(n,a,e,c=[]){var l;const r=c.length>0?`
Patient Medical Records:
${c.map(i=>`- ${i.testName||i.filename}: ${i.keyFindings||(typeof i.findings=="string"?i.findings.substring(0,300)+"...":"Available")}`).join(`
`)}`:"",s={systemInstruction:{role:"system",parts:[{text:`You are the Chief Clinical Orchestrator compiling the final board report.
...
Return strictly as JSON matching this exact structure:
{
  ...
  "questionsForClinician": ["Specific question the patient can take to a clinician"]
}`}]},contents:[{role:"user",parts:[{text:"Generate final parallel report."}]}],generationConfig:{responseMimeType:"application/json"}};try{const i=new AbortController,t=setTimeout(()=>i.abort(),6e4),o=await y(u,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(s),signal:i.signal});if(clearTimeout(t),!o.ok)throw new Error(`API Error: ${o.status}`);const h=await o.json();if((l=h.candidates)!=null&&l[0]){const p=h.candidates[0].content.parts[0].text,g=p.match(/\{[\s\S]*\}/),f=g?g[0]:p.replace(/```json/g,"").replace(/```/g,"").trim();return JSON.parse(f)}}catch{return{executiveSummary:"Due to network instability, the multi-specialist synthesis could not be completed at this time.",urgency:"Routine",topDiagnoses:[],recommendedActionPlan:[],questionsForClinician:["Are there any alternative pathways we should explore while the system reconnects?"]}}}async function B(n){var e;const a={contents:[{parts:[{text:`You are a clinical dietician AI. Analyze this food entry and return a strictly valid JSON object with the nutritional breakdown.
Entry: "${n}"
...
}`}]}],generationConfig:{responseMimeType:"application/json"}};try{const c=await y(u,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(a)});if(!c.ok)throw new Error("API Error");const r=await c.json();if((e=r.candidates)!=null&&e[0]){const s=r.candidates[0].content.parts[0].text.match(/\{[\s\S]*\}/);return s?JSON.parse(s[0]):null}}catch{return null}}async function U(n){var e;const a={contents:[{parts:[{text:`You are a clinical dietician AI. Provide exactly 2 sentences of highly personalized clinical nutritional advice.
...
3. Be practical, actionable, and culturally relevant.`}]}]};try{const c=await y(u,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(a)});if(!c.ok)throw new Error("API Error");const r=await c.json();if((e=r.candidates)!=null&&e[0])return r.candidates[0].content.parts[0].text}catch{}return"Stay hydrated and focus on hitting your daily protein goals for optimal health."}async function z(n,a=7){var d,s,l;const e=x(),c=((s=(d=e==null?void 0:e.currentSummary)==null?void 0:d.topDiagnoses)==null?void 0:s.map(i=>i.condition).join(", "))||"None",r={contents:[{parts:[{text:`You are a clinical dietician AI. Generate a strictly valid JSON ${a}-day meal plan.
Conditions: ${(n.medicalConditions||[]).join(", ")||"None"}
Active Diagnoses: ${c}
...
}`}]}]};try{const i=await y(u,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(r)});if(!i.ok)throw new Error("API Error");const t=await i.json();if((l=t.candidates)!=null&&l[0]){const h=t.candidates[0].content.parts[0].text.match(/\{[\s\S]*\}/);return h?JSON.parse(h[0]):null}}catch{return null}}async function G(n,a){var s,l,i,t;const e={age:(s=n==null?void 0:n.demographics)==null?void 0:s.age,gender:(l=n==null?void 0:n.demographics)==null?void 0:l.gender,conditions:(n==null?void 0:n.conditions)||((i=n==null?void 0:n.health)==null?void 0:i.conditions)||[],medications:((n==null?void 0:n.medications)||[]).map(o=>o.name),healthFocus:n==null?void 0:n.healthFocus},c=a.map(o=>({id:o.id,label:o.label})),d={contents:[{parts:[{text:`
...
`}]}],generationConfig:{temperature:.2,response_mime_type:"application/json"}};try{const o=await y(u,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(d)});if(!o.ok)throw new Error("API Error");const h=await o.json();if((t=h.candidates)!=null&&t[0]){const g=h.candidates[0].content.parts[0].text.match(/\{[\s\S]*\}/);return g?JSON.parse(g[0]):null}}catch{return null}}async function W(n,a,e){var d,s,l,i;const r={contents:[{parts:[{text:`
...
Patient Profile:
${JSON.stringify({age:(d=e==null?void 0:e.demographics)==null?void 0:d.age,gender:(s=e==null?void 0:e.demographics)==null?void 0:s.gender,conditions:((l=e==null?void 0:e.health)==null?void 0:l.conditions)||(e==null?void 0:e.medicalConditions)})}
...
`}]}],generationConfig:{temperature:.2,response_mime_type:"application/json"}};try{const t=await y(u,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(r)});if(!t.ok)throw new Error("API Error");const o=await t.json();if((i=o.candidates)!=null&&i[0]){const p=o.candidates[0].content.parts[0].text.match(/\\[[\\s\\S]*\\]/);return p?JSON.parse(p[0]):null}}catch{return null}}async function D(n){var c;const e={contents:[{parts:[{text:`
...
`}]}],generationConfig:{temperature:.1,response_mime_type:"application/json"}};try{const s=await y(u,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(r)});if(!s.ok)throw new Error("API Error");const l=await s.json();if((d=l.candidates)!=null&&d[0]){const t=l.candidates[0].content.parts[0].text.match(/\{[\s\S]*\}/);return t?JSON.parse(t[0]):null}}catch{return null}}async function X(n,a){var d,s,l,i;const e=a?`Patient Context: Age ${((d=a.personal)==null?void 0:d.age)||"unknown"}, Gender: ${((s=a.personal)==null?void 0:s.gender)||"unknown"}. Existing conditions: ${(((l=a.health)==null?void 0:l.conditions)||[]).join(", ")||"None"}.`:"",r={contents:[{parts:[{text:`You are an elite Clinical Pathway Simulator AI. 
...
}`}]}],generationConfig:{temperature:.2,response_mime_type:"application/json"}};try{const t=await y(u,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(r)});if(!t.ok)throw new Error("API Error");const o=await t.json();if((i=o.candidates)!=null&&i[0]){const p=o.candidates[0].content.parts[0].text.match(/\{[\s\S]*\}/);return p?JSON.parse(p[0]):null}}catch{return null}}async function K(n,a,e){var d,s,l,i;if(!n||n.length===0)return[];const r={contents:[{parts:[{text:`You are a clinical trials matching algorithm.
...
Patient Gender: ${((s=e==null?void 0:e.demographics)==null?void 0:s.gender)||"Unknown"}
...
]`}]}],generationConfig:{temperature:.1,response_mime_type:"application/json"}};try{const t=await y(u,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(r)});if(!t.ok)throw new Error("API Error");const o=await t.json();if((i=o.candidates)!=null&&i[0]){const p=o.candidates[0].content.parts[0].text.match(/\[[\s\S]*\]/),g=p?JSON.parse(p[0]):[];return n.map(f=>{const m=g.find(w=>w.id===f.id)||{};return{...f,matchScore:m.matchScore||Math.floor(Math.random()*20)+60,aiContext:m.aiContext||"Relevance based on your active clinical hypotheses."}})}}catch{}return n}async function Q(n,a,e){var d,s,l,i;if(!n||n.length===0)return[];const r={contents:[{parts:[{text:`You are a clinical research AI.
...
]`}]}],generationConfig:{temperature:.1,response_mime_type:"application/json"}};try{const t=await y(u,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(r)});if(!t.ok)throw new Error("API Error");const o=await t.json();if((i=o.candidates)!=null&&i[0]){const p=o.candidates[0].content.parts[0].text.match(/\[[\s\S]*\]/),g=p?JSON.parse(p[0]):[];return n.map(f=>{const m=g.find(w=>w.id===f.id)||{};return{...f,matchScore:m.matchScore||Math.floor(Math.random()*20)+60,aiContext:m.aiContext||"This research paper investigates biological mechanisms relevant to your hypotheses."}})}}catch{}return n}export{B as analyzeFoodEntry,_ as analyzeLabReport,Q as analyzeLiteratureRelevance,K as analyzeTrialRelevance,j as chatWithGemini,L as chatWithMDTSpecialist,R as chatWithTherapyGemini,V as checkDrugInteractions,M as fetchMedicineData,U as generateDieticianAdvice,H as generateMDTReport,z as generateMealPlan,q as generateParallelMultiReport,D as generateProfileSynthesis,F as runDebateRound,W as runDifferentialAnalysis,Y as runMDTConference,J as selectMDTSpecialists,X as simulatePathway,G as suggestSpecialists};
