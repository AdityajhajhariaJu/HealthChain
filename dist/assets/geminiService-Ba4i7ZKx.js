import{g as x,b as S}from"./index-CyCW7Evk.js";import"./framer-motion-DWOSTlfN.js";import"./react-vendor-ikJrUrLd.js";function b(){var l,t;let e=[];const a=x(),n=S();let o=`PATIENT PROFILE:
...
`),o!==`PATIENT PROFILE:
`&&e.push(o);const d=Object.entries(a.vitals.latestLabValues);if(d.length>0){let i=`LABS:
`;d.slice(0,8).forEach(([s,p])=>{i+=`- ${s}: ${p.value} ${p.unit} (${p.status})
`}),e.push(i)}if(n){let i=`ACTIVE CASE:
`;i+=`- ${n.title}
`,(l=n.intakeData)!=null&&l.chiefComplaint&&(i+=`- Concern: ${n.intakeData.chiefComplaint.substring(0,200)}
`),(t=n.report)!=null&&t.executiveSummary&&(i+=`- Synthesis: ${n.report.executiveSummary.substring(0,200)}
`);const s=(n.medicalRecords||[]).slice(0,3);s.length&&(i+=`- Evidence: ${s.map(p=>`${p.filename} (${p.keyFindings||"On file"})`).join("; ")}
`),e.push(i)}const c=a.timeline.filter(i=>["diagnosis","mdt_report","lab_report"].includes(i.type));if(c.length>0){let i=`HISTORY:
`;c.slice(0,3).forEach(s=>{i+=`- ${new Date(s.date).toLocaleDateString()}: ${s.title}
...
`),r}const f="/api/gemini",u=async(e,a={},n=6e4,o=2)=>{if(typeof navigator<"u"&&!navigator.onLine)throw new Error("Offline");let d;for(let c=0;c<=o;c++){const r=new AbortController,l=setTimeout(()=>r.abort(),n);try{const t=await fetch(e,{...a,signal:r.signal});if(clearTimeout(l),t.status===429&&c<o){await new Promise(i=>setTimeout(i,1e3*(c+1)));continue}return t}catch(t){if(clearTimeout(l),d=t,t.name==="AbortError"&&c<o)continue;if(c===o)throw t.name==="AbortError"?new Error("Timeout"):t}}throw d},T=`You are HealthChain's clinical investigation AI.
...
Do NOT include ANALYSIS_COMPLETE until you are ready to conclude.`;async function k(e){var l,t;const o=(((l=e[0])==null?void 0:l.role)==="model"?e.slice(1):e).slice(-12).map(i=>{let s=i.content;return i.role==="analysis"&&(s=`ANALYSIS_COMPLETE
\`\`\`json
${JSON.stringify(i.content)}
\`\`\``),{role:i.role==="user"?"user":"model",parts:[{text:s}]}}),d=b(),r={systemInstruction:{role:"system",parts:[{text:T+d}]},contents:o};try{const i=await u(f,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(r)});if(!i.ok)throw new Error(`API Error: ${i.status}`);const s=await i.json();return(t=s.candidates)!=null&&t[0]?s.candidates[0].content.parts[0].text:"Could you tell me a bit more about that?"}catch{return"Connection issue. Please try again."}}const C=`You are a clinical pharmacology AI.
...
If the medicine is completely unrecognized, return a JSON object with "name": "Unknown", and explain that data is unavailable in the "uses" field.`;async function I(e,a=null){var d;let n=e;a&&(n+=`

PATIENT PROFILE:
Allergies: ${a.allergies.join(", ")||"None"}
Current Medications: ${a.medications.map(c=>c.name).join(", ")||"None"}

Please strictly evaluate for interactions.`);const o={systemInstruction:{role:"system",parts:[{text:C}]},contents:[{role:"user",parts:[{text:n}]}],generationConfig:{responseMimeType:"application/json"}};try{const c=await u(f,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(o)});if(!c.ok)throw new Error(`API Error: ${c.status}`);const r=await c.json();if((d=r.candidates)!=null&&d[0]){const t=r.candidates[0].content.parts[0].text.replace(/```json/g,"").replace(/```/g,"").trim();return JSON.parse(t)}throw new Error("No candidate returned")}catch{return null}}const v=`You are Ava, HealthChain's "Medical Chief of Staff" and Personal Health Assistant.
...
`;async function E(e){var c;const a=e.slice(-12).map(r=>({role:r.role==="user"?"user":"model",parts:[{text:r.content}]})),n=b(),d={systemInstruction:{role:"system",parts:[{text:v+n}]},contents:a};try{const r=await u(f,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(d)});if(!r.ok)throw new Error(`API Error: ${r.status}`);const l=await r.json();return(c=l.candidates)!=null&&c[0]?l.candidates[0].content.parts[0].text:"I'm here for you. Could you tell me a bit more?"}catch{return"I'm having a little trouble connecting right now, but I'm still here for you."}}const A=`You are HealthChain's "Clinical Lab Interpreter", a highly advanced medical AI capable of reading lab reports, blood work, MRIs, and prescriptions.
...
If no document is provided or it is unreadable, return a JSON object with "testName": "Unrecognized / No Document", and explain the issue in "interpretation".`;async function $(e,a,n){var c,r,l;const d={systemInstruction:{role:"system",parts:[{text:`${A}
...
(Use this patient context strictly for determining the correct normal reference ranges for lab vitals like testosterone, eGFR, hemoglobin, etc.)`}]},contents:[{role:"user",parts:[{text:"Analyze this clinical report."},{inlineData:{mimeType:a,data:e}}]}],generationConfig:{responseMimeType:"application/json"}};try{const t=await u(f,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(d)});if(!t.ok)throw new Error(`API Error: ${t.status}`);const i=await t.json();if((l=i.candidates)!=null&&l[0]){const p=i.candidates[0].content.parts[0].text.replace(/```json/g,"").replace(/```/g,"").trim();return JSON.parse(p)}throw new Error("No candidate returned")}catch{return null}}async function j(e){var o;const n={systemInstruction:{role:"system",parts:[{text:`You are a medical triage AI. Based on the patient's chief complaint, select the 3 to 5 most appropriate medical specialists to form a Multi-Disciplinary Team (MDT) board.
Chief Complaint: "${e}"
...
`)}
Ask targeted questions to confirm or rule out these active hypotheses.`:"",p=i+l+s,m=e.slice(-12).map(h=>({role:h.role==="user"?"user":"model",parts:[{text:h.text||h.content}]})),y={systemInstruction:{role:"system",parts:[{text:p}]},contents:m,generationConfig:{responseMimeType:"application/json",responseSchema:{type:"object",properties:{internalThoughts:{type:"string"},currentHypotheses:{type:"array",items:{type:"string"}},response:{type:"string"},widgetType:{type:"string"},widgetOptions:{type:"array",items:{type:"string"}}},required:["internalThoughts","currentHypotheses","response"]}}};try{const h=await u(f,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(y)});if(!h.ok)throw new Error(`API Error: ${h.status}`);const w=await h.json();return(g=w.candidates)!=null&&g[0]?w.candidates[0].content.parts[0].text.trim():'{"response": "Could you tell me more?", "internalThoughts": "Awaiting more info", "currentHypotheses": []}'}catch{return'{"response": "I am experiencing network issues.", "internalThoughts": "Network error", "currentHypotheses": []}'}}async function _(e,a,n=[]){var l;const o=n.length>0?`
Patient Medical Records:
${n.map(t=>`- ${t.testName||t.filename}: ${t.keyFindings||(typeof t.findings=="string"?t.findings.substring(0,300)+"...":"Available")}`).join(`
`)}`:"",d=Object.fromEntries(Object.entries(a).map(([t,i])=>[t,i.map(s=>({role:s.role,text:s.text,hypotheses:s.currentHypotheses}))])),r={systemInstruction:{role:"system",parts:[{text:`You are the Chief Clinical Orchestrator for an MDT board.
...
}`}]},contents:[{role:"user",parts:[{text:"Run the MDT Conference based on the provided specialist data."}]}],generationConfig:{responseMimeType:"application/json"}};try{const t=await u(f,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(r)});if(!t.ok)throw new Error(`API Error: ${t.status}`);const i=await t.json();if((l=i.candidates)!=null&&l[0]){const p=i.candidates[0].content.parts[0].text.replace(/```json/g,"").replace(/```/g,"").trim();return JSON.parse(p)}}catch{return{corroborations:[],contentions:[],followUpQuestions:[],debateSummary:"MDT consensus failed due to an error."}}return null}async function R(e,a,n,o=[]){var l;const d=o.length>0?`
Patient Medical Records:
${o.map(t=>`- ${t.testName||t.filename}: ${t.keyFindings||(typeof t.findings=="string"?t.findings.substring(0,300)+"...":"Available")}`).join(`
`)}`:"",r={systemInstruction:{role:"system",parts:[{text:`You are the Chief Clinical Orchestrator compiling the final MDT report.
Patient Intake: ${e.chiefComplaint}${d}
...
Return strictly as JSON matching this exact structure:
{
  ...
  "questionsForClinician": ["Specific question the patient can take to a clinician"]
}`}]},contents:[{role:"user",parts:[{text:"Generate final parallel report."}]}],generationConfig:{responseMimeType:"application/json"}};try{const t=new AbortController,i=setTimeout(()=>t.abort(),6e4),s=await u(f,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(r),signal:t.signal});if(clearTimeout(i),!s.ok)throw new Error(`API Error: ${s.status}`);const p=await s.json();if((l=p.candidates)!=null&&l[0]){const m=p.candidates[0].content.parts[0].text,y=m.match(/\{[\s\S]*\}/),g=y?y[0]:m.replace(/```json/g,"").replace(/```/g,"").trim();return JSON.parse(g)}}catch{return{executiveSummary:"Due to network instability, the multi-specialist synthesis could not be completed at this time.",urgency:"Routine",topDiagnoses:[],recommendedActionPlan:[],questionsForClinician:["Are there any alternative pathways we should explore while the system reconnects?"]}}}async function Y(e){var n;const a={contents:[{parts:[{text:`You are a clinical dietician AI. Analyze this food entry and return a strictly valid JSON object with the nutritional breakdown.
Entry: "${e}"
...
}`}]}],generationConfig:{responseMimeType:"application/json"}};try{const o=await u(f,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(a)});if(!o.ok)throw new Error("API Error");const d=await o.json();if((n=d.candidates)!=null&&n[0]){const r=d.candidates[0].content.parts[0].text.match(/\{[\s\S]*\}/);return r?JSON.parse(r[0]):null}}catch{return null}}async function H(e){var n;const a={contents:[{parts:[{text:`You are a clinical dietician AI. Provide exactly 2 sentences of highly personalized clinical nutritional advice.
...
3. Be practical, actionable, and culturally relevant.`}]}]};try{const o=await u(f,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(a)});if(!o.ok)throw new Error("API Error");const d=await o.json();if((n=d.candidates)!=null&&n[0])return d.candidates[0].content.parts[0].text}catch{}return"Stay hydrated and focus on hitting your daily protein goals for optimal health."}async function F(e,a=7){var c,r,l;const n=S(),o=((r=(c=n==null?void 0:n.currentSummary)==null?void 0:c.topDiagnoses)==null?void 0:r.map(t=>t.condition).join(", "))||"None",d={contents:[{parts:[{text:`You are a clinical dietician AI. Generate a strictly valid JSON ${a}-day meal plan.
Conditions: ${(e.medicalConditions||[]).join(", ")||"None"}
Active Diagnoses: ${o}
...
}`}]}]};try{const t=await u(f,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(d)});if(!t.ok)throw new Error("API Error");const i=await t.json();if((l=i.candidates)!=null&&l[0]){const p=i.candidates[0].content.parts[0].text.match(/\{[\s\S]*\}/);return p?JSON.parse(p[0]):null}}catch{return null}}async function q(e,a){var l,t,i,s;const n="https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=AIzaSyCAQYkzKXArqDV-_CIx0fQzLfLj5n0fmys",o={age:(l=e==null?void 0:e.demographics)==null?void 0:l.age,gender:(t=e==null?void 0:e.demographics)==null?void 0:t.gender,conditions:(e==null?void 0:e.conditions)||((i=e==null?void 0:e.health)==null?void 0:i.conditions)||[],medications:((e==null?void 0:e.medications)||[]).map(p=>p.name),healthFocus:e==null?void 0:e.healthFocus},d=a.map(p=>({id:p.id,label:p.label})),r={contents:[{parts:[{text:`
...
`}]}],generationConfig:{temperature:.2,response_mime_type:"application/json"}};try{const p=await u(n,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(r)});if(!p.ok)throw new Error("API Error");const m=await p.json();if((s=m.candidates)!=null&&s[0]){const g=m.candidates[0].content.parts[0].text.match(/\{[\s\S]*\}/);return g?JSON.parse(g[0]):null}}catch{return null}}async function U(e,a,n){var r,l,t,i;const o="https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=AIzaSyCAQYkzKXArqDV-_CIx0fQzLfLj5n0fmys",c={contents:[{parts:[{text:`
...
Patient Profile:
${JSON.stringify({age:(r=n==null?void 0:n.demographics)==null?void 0:r.age,gender:(l=n==null?void 0:n.demographics)==null?void 0:l.gender,conditions:((t=n==null?void 0:n.health)==null?void 0:t.conditions)||(n==null?void 0:n.medicalConditions)})}
...
`}]}],generationConfig:{temperature:.2,response_mime_type:"application/json"}};try{const s=await u(o,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(c)});if(!s.ok)throw new Error("API Error");const p=await s.json();if((i=p.candidates)!=null&&i[0]){const y=p.candidates[0].content.parts[0].text.match(/\\[[\\s\\S]*\\]/);return y?JSON.parse(y[0]):null}}catch{return null}}async function z(e){var d;const a="https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=AIzaSyCAQYkzKXArqDV-_CIx0fQzLfLj5n0fmys",o={contents:[{parts:[{text:`
...
`}]}],generationConfig:{temperature:.3,response_mime_type:"application/json"}};try{const c=await u(a,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(o)});if(!c.ok)throw new Error("API Error");const r=await c.json();if((d=r.candidates)!=null&&d[0]){const t=r.candidates[0].content.parts[0].text.match(/\{[\s\S]*\}/);return t?JSON.parse(t[0]):null}}catch{return null}}async function D(e,a){var r;const n="https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=AIzaSyCAQYkzKXArqDV-_CIx0fQzLfLj5n0fmys",o=a.map(l=>l.name).join(", "),c={contents:[{parts:[{text:`
...
`}]}],generationConfig:{temperature:.1,response_mime_type:"application/json"}};try{const l=await u(n,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(c)});if(!l.ok)throw new Error("API Error");const t=await l.json();if((r=t.candidates)!=null&&r[0]){const s=t.candidates[0].content.parts[0].text.match(/\{[\s\S]*\}/);return s?JSON.parse(s[0]):null}}catch{return null}}async function B(e,a){var c,r,l,t;const n=a?`Patient Context: Age ${((c=a.personal)==null?void 0:c.age)||"unknown"}, Gender: ${((r=a.personal)==null?void 0:r.gender)||"unknown"}. Existing conditions: ${(((l=a.health)==null?void 0:l.conditions)||[]).join(", ")||"None"}.`:"",d={contents:[{parts:[{text:`You are an elite Clinical Pathway Simulator AI. 
...
}`}]}],generationConfig:{temperature:.2,response_mime_type:"application/json"}};try{const i=await u(f,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(d)});if(!i.ok)throw new Error("API Error");const s=await i.json();if((t=s.candidates)!=null&&t[0]){const m=s.candidates[0].content.parts[0].text.match(/\{[\s\S]*\}/);return m?JSON.parse(m[0]):null}}catch{return null}}async function G(e,a,n){var c,r,l,t;if(!e||e.length===0)return[];const d={contents:[{parts:[{text:`You are a clinical trials matching algorithm.
...
]`}]}],generationConfig:{temperature:.1,response_mime_type:"application/json"}};try{const i=await u(f,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(d)});if(!i.ok)throw new Error("API Error");const s=await i.json();if((t=s.candidates)!=null&&t[0]){const m=s.candidates[0].content.parts[0].text.match(/\[[\s\S]*\]/),y=m?JSON.parse(m[0]):[];return e.map(g=>{const h=y.find(w=>w.id===g.id)||{};return{...g,matchScore:h.matchScore||Math.floor(Math.random()*20)+60,aiContext:h.aiContext||"Relevance based on your active clinical hypotheses."}})}}catch{}return e}async function W(e,a,n){var c,r,l,t;if(!e||e.length===0)return[];const d={contents:[{parts:[{text:`You are a clinical research AI.
