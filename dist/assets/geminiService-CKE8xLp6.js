import{g as T,b as S}from"./index-DSS8fwFC.js";import"./framer-motion-DWOSTlfN.js";import"./react-vendor-ikJrUrLd.js";function b(){var l,t;let n=[];const a=T(),i=S();let r=`PATIENT PROFILE:
...
Do NOT include ANALYSIS_COMPLETE until you are ready to conclude.`;async function k(n){var l,t;const r=(((l=n[0])==null?void 0:l.role)==="model"?n.slice(1):n).slice(-12).map(e=>{let c=e.content;return e.role==="analysis"&&(c=`ANALYSIS_COMPLETE
\`\`\`json
...
}`}]}]};try{const t=await y(m,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(d)});if(!t.ok)throw new Error("API Error");const e=await t.json();if((l=e.candidates)!=null&&l[0]){const p=e.candidates[0].content.parts[0].text.match(/\{[\s\S]*\}/);return p?JSON.parse(p[0]):null}}catch{return null}}async function q(n,a){var s,l,t,e;const i={age:(s=n==null?void 0:n.demographics)==null?void 0:s.age,gender:(l=n==null?void 0:n.demographics)==null?void 0:l.gender,conditions:(n==null?void 0:n.conditions)||((t=n==null?void 0:n.health)==null?void 0:t.conditions)||[],medications:((n==null?void 0:n.medications)||[]).map(c=>c.name),healthFocus:n==null?void 0:n.healthFocus},r=a.map(c=>({id:c.id,label:c.label})),o={contents:[{parts:[{text:`
...
`}]}],generationConfig:{temperature:.2,response_mime_type:"application/json"}};try{const c=await y(m,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(o)});if(!c.ok)throw new Error("API Error");const p=await c.json();if((e=p.candidates)!=null&&e[0]){const g=p.candidates[0].content.parts[0].text.match(/\{[\s\S]*\}/);return g?JSON.parse(g[0]):null}}catch{return null}}async function U(n,a,i){var o,s,l,t;const d={contents:[{parts:[{text:`
...
`}]}],generationConfig:{temperature:.2,response_mime_type:"application/json"}};try{const e=await y(m,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(d)});if(!e.ok)throw new Error("API Error");const c=await e.json();if((t=c.candidates)!=null&&t[0]){const u=c.candidates[0].content.parts[0].text.match(/\\[[\\s\\S]*\\]/);return u?JSON.parse(u[0]):null}}catch{return null}}async function B(n){var r;const i={contents:[{parts:[{text:`
...
`}]}],generationConfig:{temperature:.1,response_mime_type:"application/json"}};try{const s=await y(m,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(d)});if(!s.ok)throw new Error("API Error");const l=await s.json();if((o=l.candidates)!=null&&o[0]){const e=l.candidates[0].content.parts[0].text.match(/\{[\s\S]*\}/);return e?JSON.parse(e[0]):null}}catch{return null}}async function z(n,a){var o,s,l,t;const i=a?`Patient Context: Age ${((o=a.personal)==null?void 0:o.age)||"unknown"}, Gender: ${((s=a.personal)==null?void 0:s.gender)||"unknown"}. Existing conditions: ${(((l=a.health)==null?void 0:l.conditions)||[]).join(", ")||"None"}.`:"",d={contents:[{parts:[{text:`You are an elite Clinical Pathway Simulator AI. 
...
}`}]}],generationConfig:{temperature:.2,response_mime_type:"application/json"}};try{const e=await y(m,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(d)});if(!e.ok)throw new Error("API Error");const c=await e.json();if((t=c.candidates)!=null&&t[0]){const u=c.candidates[0].content.parts[0].text.match(/\{[\s\S]*\}/);return u?JSON.parse(u[0]):null}}catch{return null}}async function G(n,a,i){var o,s,l,t;if(!n||n.length===0)return[];const d={contents:[{parts:[{text:`You are a clinical trials matching algorithm.
...
]`}]}],generationConfig:{temperature:.1,response_mime_type:"application/json"}};try{const e=await y(m,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(d)});if(!e.ok)throw new Error("API Error");const c=await e.json();if((t=c.candidates)!=null&&t[0]){const u=c.candidates[0].content.parts[0].text.match(/\[[\s\S]*\]/),g=u?JSON.parse(u[0]):[];return n.map(f=>{const h=g.find(w=>w.id===f.id)||{};return{...f,matchScore:h.matchScore||Math.floor(Math.random()*20)+60,aiContext:h.aiContext||"Relevance based on your active clinical hypotheses."}})}}catch{}return n}async function W(n,a,i){var o,s,l,t;if(!n||n.length===0)return[];const d={contents:[{parts:[{text:`You are a clinical research AI.
