import{g as T,b as S}from"./index-C4BhzpgL.js";import"./framer-motion-CDW5Q6YF.js";import"./react-vendor-DURbyAIq.js";import"./pdf-tools-BU1OiuD_.js";import"./supabase-BddhP1o-.js";function b(){var l,t;let n=[];const a=T(),i=S();let r=`PATIENT PROFILE:
...
`),a.medications&&a.medications.length>0&&(r+=`- Meds: ${a.medications.map(e=>e.name).join(", ")}
`),a.familyHistory&&a.familyHistory.length>0&&(r+=`- Family Hx: ${a.familyHistory.join(", ")}
`),r!==`PATIENT PROFILE:
`&&n.push(r);const d=Object.entries(a.vitals.latestLabValues);if(d.length>0){let e=`LABS:
`;d.slice(0,8).forEach(([c,p])=>{e+=`- ${c}: ${p.value} ${p.unit} (${p.status})
`}),n.push(e)}if(i){let e=`ACTIVE CASE:
`;e+=`- ${i.title}
`,(l=i.intakeData)!=null&&l.chiefComplaint&&(e+=`- Concern: ${i.intakeData.chiefComplaint.substring(0,200)}
`),(t=i.report)!=null&&t.executiveSummary&&(e+=`- Synthesis: ${i.report.executiveSummary.substring(0,200)}
`);const c=(i.medicalRecords||[]).slice(0,3);c.length&&(e+=`- Evidence: ${c.map(p=>`${p.filename} (${p.keyFindings||"On file"})`).join("; ")}
`),n.push(e)}const o=a.timeline.filter(e=>["diagnosis","mdt_report","lab_report"].includes(e.type));if(o.length>0){let e=`HISTORY:
`;o.slice(0,3).forEach(c=>{e+=`- ${new Date(c.date).toLocaleDateString()}: ${c.title}
`}),n.push(e)}if(n.length===0)return"";let s=`

=== PATIENT CONTEXT ===
${n.join(`
`)}========================
`;return s.length>1500&&(s=s.substring(0,1497)+`...
`),s}const u="/api/gemini",y=async(n,a={},i=6e4,r=2)=>{if(typeof navigator<"u"&&!navigator.onLine)throw new Error("Offline");try{const l=Date.now();let t=[];try{t=JSON.parse(localStorage.getItem("hc_api_logs")||"[]")}catch{t=[]}if(t=t.filter(e=>l-e<36e5),t.length>=20)throw new Error("Rate limit exceeded. Please try again later.");t.push(l),localStorage.setItem("hc_api_logs",JSON.stringify(t))}catch(o){if(o.message.includes("Rate limit exceeded"))throw o}let d;for(let o=0;o<=r;o++){const s=new AbortController,l=setTimeout(()=>s.abort(),i);try{const t=await fetch(n,{...a,signal:s.signal});if(clearTimeout(l),t.status===429&&o<r){await new Promise(e=>setTimeout(e,1e3*(o+1)));continue}if(!t.ok&&t.status>=500&&o<r){await new Promise(e=>setTimeout(e,1e3*Math.pow(2,o)));continue}return t}catch(t){if(clearTimeout(l),d=t,(t.name==="AbortError"||t.name==="TypeError")&&o<r){await new Promise(e=>setTimeout(e,1e3*Math.pow(2,o)));continue}if(o===r)throw t.name==="AbortError"?new Error("Timeout"):t}}throw d},x=`You are HealthChain's clinical investigation AI.
...
Do NOT include ANALYSIS_COMPLETE until you are ready to conclude.`;async function I(n){var l,t;const r=(((l=n[0])==null?void 0:l.role)==="model"?n.slice(1):n).slice(-12).map(e=>{let c=e.content;return e.role==="analysis"&&(c=`ANALYSIS_COMPLETE
\`\`\`json
${JSON.stringify(e.content)}
\`\`\``),{role:e.role==="user"?"user":"model",parts:[{text:c}]}}),d=b(),s={systemInstruction:{role:"system",parts:[{text:x+d}]},contents:r};try{const e=await y(u,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(s)});if(!e.ok)throw new Error(`API Error: ${e.status}`);const c=await e.json();return(t=c.candidates)!=null&&t[0]?c.candidates[0].content.parts[0].text:"Could you tell me a bit more about that?"}catch{return"Connection issue. Please try again."}}const C=`You are a clinical pharmacology AI.
...
If no document is provided or it is unreadable, return a JSON object with "testName": "Unrecognized / No Document", and explain the issue in "interpretation".`;async function M(n,a,i){var o,s,l;const d={systemInstruction:{role:"system",parts:[{text:`${P}
...
(Use this patient context strictly for determining the correct normal reference ranges for lab vitals like testosterone, eGFR, hemoglobin, etc.)`}]},contents:[{role:"user",parts:[{text:"Analyze this clinical report."},{inlineData:{mimeType:a,data:n}}]}],generationConfig:{responseMimeType:"application/json"}};try{const t=await y(u,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(d)});if(!t.ok)throw new Error(`API Error: ${t.status}`);const e=await t.json();if((l=e.candidates)!=null&&l[0]){const p=e.candidates[0].content.parts[0].text.replace(/```json/g,"").replace(/```/g,"").trim();return JSON.parse(p)}throw new Error("No candidate returned")}catch{return null}}async function R(n){var r;const i={systemInstruction:{role:"system",parts:[{text:`You are a medical triage AI. Based on the patient's chief complaint, select the 3 to 5 most appropriate medical specialists to form a Multi-Disciplinary Team (MDT) board.
Chief Complaint: "${n}"
...
`)}
Ask targeted questions to confirm or rule out these active hypotheses.`:"",p=e+l+c,m=n.slice(-12).map(h=>({role:h.role==="user"?"user":"model",parts:[{text:h.text||h.content}]})),g={systemInstruction:{role:"system",parts:[{text:p}]},contents:m,generationConfig:{responseMimeType:"application/json",responseSchema:{type:"object",properties:{internalThoughts:{type:"string"},currentHypotheses:{type:"array",items:{type:"string"}},response:{type:"string"},widgetType:{type:"string"},widgetOptions:{type:"array",items:{type:"string"}}},required:["internalThoughts","currentHypotheses","response"]}}};try{const h=await y(u,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(g)});if(!h.ok)throw new Error(`API Error: ${h.status}`);const w=await h.json();return(f=w.candidates)!=null&&f[0]?w.candidates[0].content.parts[0].text.trim():'{"response": "Could you tell me more?", "internalThoughts": "Awaiting more info", "currentHypotheses": []}'}catch{return'{"response": "I am experiencing network issues.", "internalThoughts": "Network error", "currentHypotheses": []}'}}async function J(n,a,i=[]){var l;const r=i.length>0?`
Patient Medical Records:
${i.map(t=>`- ${t.testName||t.filename}: ${t.keyFindings||(typeof t.findings=="string"?t.findings.substring(0,300)+"...":"Available")}`).join(`
`)}`:"",d=Object.fromEntries(Object.entries(a).map(([t,e])=>[t,e.map(c=>({role:c.role,text:c.text,hypotheses:c.currentHypotheses}))])),s={systemInstruction:{role:"system",parts:[{text:`You are the Chief Clinical Orchestrator for an MDT board.
...
}`}]},contents:[{role:"user",parts:[{text:"Run the MDT Conference based on the provided specialist data."}]}],generationConfig:{responseMimeType:"application/json"}};try{const t=await y(u,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(s)});if(!t.ok)throw new Error(`API Error: ${t.status}`);const e=await t.json();if((l=e.candidates)!=null&&l[0]){const p=e.candidates[0].content.parts[0].text.replace(/```json/g,"").replace(/```/g,"").trim();return JSON.parse(p)}}catch{return{corroborations:[],contentions:[],followUpQuestions:[],debateSummary:"MDT consensus failed due to an error."}}return null}async function L(n,a,i,r=[]){var l;const d=r.length>0?`
Patient Medical Records:
${r.map(t=>`- ${t.testName||t.filename}: ${t.keyFindings||(typeof t.findings=="string"?t.findings.substring(0,300)+"...":"Available")}`).join(`
...
--- Patient Medical Records ---
${i.map(t=>`File: ${t.testName||t.filename}
...
Return strictly as JSON matching this exact structure:
{
  ...
  "questionsForClinician": ["Specific question the patient can take to a clinician"]
}`}]},contents:[{role:"user",parts:[{text:"Generate final parallel report."}]}],generationConfig:{responseMimeType:"application/json"}};try{const t=new AbortController,e=setTimeout(()=>t.abort(),6e4),c=await y(u,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(s),signal:t.signal});if(clearTimeout(e),!c.ok)throw new Error(`API Error: ${c.status}`);const p=await c.json();if((l=p.candidates)!=null&&l[0]){const m=p.candidates[0].content.parts[0].text,g=m.match(/\{[\s\S]*\}/),f=g?g[0]:m.replace(/```json/g,"").replace(/```/g,"").trim();return JSON.parse(f)}}catch{return{executiveSummary:"Due to network instability, the multi-specialist synthesis could not be completed at this time.",urgency:"Routine",topDiagnoses:[],recommendedActionPlan:[],questionsForClinician:["Are there any alternative pathways we should explore while the system reconnects?"]}}}async function F(n){var i;const a={contents:[{parts:[{text:`You are a clinical dietician AI. Analyze this food entry and return a strictly valid JSON object with the nutritional breakdown.
Entry: "${n}"
...
}`}]}],generationConfig:{responseMimeType:"application/json"}};try{const r=await y(u,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(a)});if(!r.ok)throw new Error("API Error");const d=await r.json();if((i=d.candidates)!=null&&i[0]){const s=d.candidates[0].content.parts[0].text.match(/\{[\s\S]*\}/);return s?JSON.parse(s[0]):null}}catch{return null}}async function q(n){var i;const a={contents:[{parts:[{text:`You are a clinical dietician AI. Provide exactly 2 sentences of highly personalized clinical nutritional advice.
...
3. Be practical, actionable, and culturally relevant.`}]}]};try{const r=await y(u,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(a)});if(!r.ok)throw new Error("API Error");const d=await r.json();if((i=d.candidates)!=null&&i[0])return d.candidates[0].content.parts[0].text}catch{}return"Stay hydrated and focus on hitting your daily protein goals for optimal health."}async function U(n,a=7){var o,s,l;const i=S(),r=((s=(o=i==null?void 0:i.currentSummary)==null?void 0:o.topDiagnoses)==null?void 0:s.map(t=>t.condition).join(", "))||"None",d={contents:[{parts:[{text:`You are a clinical dietician AI. Generate a strictly valid JSON ${a}-day meal plan.
Conditions: ${(n.medicalConditions||[]).join(", ")||"None"}
Active Diagnoses: ${r}
...
}`}]}]};try{const t=await y(u,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(d)});if(!t.ok)throw new Error("API Error");const e=await t.json();if((l=e.candidates)!=null&&l[0]){const p=e.candidates[0].content.parts[0].text.match(/\{[\s\S]*\}/);return p?JSON.parse(p[0]):null}}catch{return null}}async function B(n,a){var s,l,t,e;const i={age:(s=n==null?void 0:n.demographics)==null?void 0:s.age,gender:(l=n==null?void 0:n.demographics)==null?void 0:l.gender,conditions:(n==null?void 0:n.conditions)||((t=n==null?void 0:n.health)==null?void 0:t.conditions)||[],medications:((n==null?void 0:n.medications)||[]).map(c=>c.name),healthFocus:n==null?void 0:n.healthFocus},r=a.map(c=>({id:c.id,label:c.label})),o={contents:[{parts:[{text:`
...
`}]}],generationConfig:{temperature:.2,response_mime_type:"application/json"}};try{const c=await y(u,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(o)});if(!c.ok)throw new Error("API Error");const p=await c.json();if((e=p.candidates)!=null&&e[0]){const g=p.candidates[0].content.parts[0].text.match(/\{[\s\S]*\}/);return g?JSON.parse(g[0]):null}}catch{return null}}async function D(n,a,i){var o,s,l,t;const d={contents:[{parts:[{text:`
...
${JSON.stringify(a.map(e=>({test:e.testName||e.filename,findings:e.keyFindings||(typeof e.findings=="string"?e.findings.substring(0,300)+"...":"Available"),abnormal:e.abnormalities})))}
...
`}]}],generationConfig:{temperature:.2,response_mime_type:"application/json"}};try{const e=await y(u,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(d)});if(!e.ok)throw new Error("API Error");const c=await e.json();if((t=c.candidates)!=null&&t[0]){const m=c.candidates[0].content.parts[0].text.match(/\\[[\\s\\S]*\\]/);return m?JSON.parse(m[0]):null}}catch{return null}}async function z(n){var r;const i={contents:[{parts:[{text:`
...
`}]}],generationConfig:{temperature:.3,response_mime_type:"application/json"}};try{const d=await y(u,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(i)});if(!d.ok)throw new Error("API Error");const o=await d.json();if((r=o.candidates)!=null&&r[0]){const l=o.candidates[0].content.parts[0].text.match(/\{[\s\S]*\}/);return l?JSON.parse(l[0]):null}}catch{return null}}async function G(n,a){var o;const i=a.map(s=>s.name).join(", "),d={contents:[{parts:[{text:`
...
`}]}],generationConfig:{temperature:.1,response_mime_type:"application/json"}};try{const s=await y(u,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(d)});if(!s.ok)throw new Error("API Error");const l=await s.json();if((o=l.candidates)!=null&&o[0]){const e=l.candidates[0].content.parts[0].text.match(/\{[\s\S]*\}/);return e?JSON.parse(e[0]):null}}catch{return null}}async function W(n,a){var o,s,l,t;const i=a?`Patient Context: Age ${((o=a.personal)==null?void 0:o.age)||"unknown"}, Gender: ${((s=a.personal)==null?void 0:s.gender)||"unknown"}. Existing conditions: ${(((l=a.health)==null?void 0:l.conditions)||[]).join(", ")||"None"}.`:"",d={contents:[{parts:[{text:`You are an elite Clinical Pathway Simulator AI. 
...
}`}]}],generationConfig:{temperature:.2,response_mime_type:"application/json"}};try{const e=await y(u,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(d)});if(!e.ok)throw new Error("API Error");const c=await e.json();if((t=c.candidates)!=null&&t[0]){const m=c.candidates[0].content.parts[0].text.match(/\{[\s\S]*\}/);return m?JSON.parse(m[0]):null}}catch{return null}}async function V(n,a,i){var o,s,l,t;if(!n||n.length===0)return[];const d={contents:[{parts:[{text:`You are a clinical trials matching algorithm.
...
Current Active Diagnoses/Hypotheses: ${JSON.stringify(((a==null?void 0:a.differentials)||[]).map(e=>e.condition))}
...
]`}]}],generationConfig:{temperature:.1,response_mime_type:"application/json"}};try{const e=await y(u,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(d)});if(!e.ok)throw new Error("API Error");const c=await e.json();if((t=c.candidates)!=null&&t[0]){const m=c.candidates[0].content.parts[0].text.match(/\[[\s\S]*\]/),g=m?JSON.parse(m[0]):[];return n.map(f=>{const h=g.find(w=>w.id===f.id)||{};return{...f,matchScore:h.matchScore||Math.floor(Math.random()*20)+60,aiContext:h.aiContext||"Relevance based on your active clinical hypotheses."}})}}catch{}return n}async function X(n,a,i){var o,s,l,t;if(!n||n.length===0)return[];const d={contents:[{parts:[{text:`You are a clinical research AI.
...
Papers to analyze:
${JSON.stringify(n.map(e=>({id:e.id,title:e.title,abstract:e.abstract})))}
...
]`}]}],generationConfig:{temperature:.1,response_mime_type:"application/json"}};try{const e=await y(u,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(d)});if(!e.ok)throw new Error("API Error");const c=await e.json();if((t=c.candidates)!=null&&t[0]){const m=c.candidates[0].content.parts[0].text.match(/\[[\s\S]*\]/),g=m?JSON.parse(m[0]):[];return n.map(f=>{const h=g.find(w=>w.id===f.id)||{};return{...f,matchScore:h.matchScore||Math.floor(Math.random()*20)+60,aiContext:h.aiContext||"This research paper investigates biological mechanisms relevant to your hypotheses."}})}}catch{}return n}export{F as analyzeFoodEntry,M as analyzeLabReport,X as analyzeLiteratureRelevance,V as analyzeTrialRelevance,I as chatWithGemini,_ as chatWithMDTSpecialist,j as chatWithTherapyGemini,G as checkDrugInteractions,$ as fetchMedicineData,q as generateDieticianAdvice,L as generateMDTReport,U as generateMealPlan,H as generateParallelMultiReport,z as generateProfileSynthesis,Y as runDebateRound,D as runDifferentialAnalysis,J as runMDTConference,R as selectMDTSpecialists,W as simulatePathway,B as suggestSpecialists};
