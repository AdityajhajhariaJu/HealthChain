import{g as x,b as S}from"./index-B_2tNSJF.js";import"./framer-motion-J0OJkgzt.js";import"./react-vendor-BNplnDmj.js";function b(){var d,n;let e=[];const a=x(),t=S();let o=`PATIENT PROFILE:
...
`),r}const T="AIzaSyCAQYkzKXArqDV-_CIx0fQzLfLj5n0fmys",f=`https://generativelanguage.googleapis.com/v1beta/models/gemini-3.5-flash:generateContent?key=${T}`,u=async(e,a={},t=6e4)=>{if(typeof navigator<"u"&&!navigator.onLine)throw new Error("Offline");const o=new AbortController,c=setTimeout(()=>o.abort(),t);try{const l=await fetch(e,{...a,signal:o.signal});return clearTimeout(c),l}catch(l){throw clearTimeout(c),l.name==="AbortError"?new Error("Timeout"):l}},C=`You are HealthChain's clinical investigation AI.
...
Do NOT include ANALYSIS_COMPLETE until you are ready to conclude.`;async function N(e){var d,n;const o=(((d=e[0])==null?void 0:d.role)==="model"?e.slice(1):e).slice(-12).map(i=>{let s=i.content;return i.role==="analysis"&&(s=`ANALYSIS_COMPLETE
\`\`\`json
${JSON.stringify(i.content)}
\`\`\``),{role:i.role==="user"?"user":"model",parts:[{text:s}]}}),c=b(),r={systemInstruction:{role:"system",parts:[{text:C+c}]},contents:o};try{const i=await u(f,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(r)});if(!i.ok)throw new Error(`API Error: ${i.status}`);const s=await i.json();return(n=s.candidates)!=null&&n[0]?s.candidates[0].content.parts[0].text:"Could you tell me a bit more about that?"}catch{return"Connection issue. Please try again."}}const A=`You are a clinical pharmacology AI.
...
`;async function $(e){var l;const a=e.slice(-12).map(r=>({role:r.role==="user"?"user":"model",parts:[{text:r.content}]})),t=b(),c={systemInstruction:{role:"system",parts:[{text:v+t}]},contents:a};try{const r=await u(f,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(c)});if(!r.ok)throw new Error(`API Error: ${r.status}`);const d=await r.json();return(l=d.candidates)!=null&&l[0]?d.candidates[0].content.parts[0].text:"I'm here for you. Could you tell me a bit more?"}catch{return"I'm having a little trouble connecting right now, but I'm still here for you."}}const P=`You are HealthChain's "Clinical Lab Interpreter", a highly advanced medical AI capable of reading lab reports, blood work, MRIs, and prescriptions.
...
If no document is provided or it is unreadable, return a JSON object with "testName": "Unrecognized / No Document", and explain the issue in "interpretation".`;async function j(e,a,t){var l,r,d;const c={systemInstruction:{role:"system",parts:[{text:`${P}
...
Return strictly as JSON matching this exact structure:
{
  ...
  "questionsForClinician": ["Specific question the patient can take to a clinician"]
}`}]},contents:[{role:"user",parts:[{text:"Generate final parallel report."}]}],generationConfig:{responseMimeType:"application/json"}};try{const n=new AbortController,i=setTimeout(()=>n.abort(),6e4),s=await u(f,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(r),signal:n.signal});if(clearTimeout(i),!s.ok)throw new Error(`API Error: ${s.status}`);const p=await s.json();if((d=p.candidates)!=null&&d[0]){const m=p.candidates[0].content.parts[0].text,y=m.match(/\{[\s\S]*\}/),g=y?y[0]:m.replace(/```json/g,"").replace(/```/g,"").trim();return JSON.parse(g)}}catch{return{executiveSummary:"Due to network instability, the multi-specialist synthesis could not be completed at this time.",urgency:"Routine",topDiagnoses:[],recommendedActionPlan:[],questionsForClinician:["Are there any alternative pathways we should explore while the system reconnects?"]}}}async function H(e){var t;const a={contents:[{parts:[{text:`You are a clinical dietician AI. Analyze this food entry and return a strictly valid JSON object with the nutritional breakdown.
Entry: "${e}"
...
}`}]}],generationConfig:{responseMimeType:"application/json"}};try{const o=await u(f,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(a)});if(!o.ok)throw new Error("API Error");const c=await o.json();if((t=c.candidates)!=null&&t[0]){const r=c.candidates[0].content.parts[0].text.match(/\{[\s\S]*\}/);return r?JSON.parse(r[0]):null}}catch{return null}}async function F(e){var t;const a={contents:[{parts:[{text:`You are a clinical dietician AI. Provide exactly 2 sentences of highly personalized clinical nutritional advice.
...
}`}]}]};try{const n=await u(f,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(c)});if(!n.ok)throw new Error("API Error");const i=await n.json();if((d=i.candidates)!=null&&d[0]){const p=i.candidates[0].content.parts[0].text.match(/\{[\s\S]*\}/);return p?JSON.parse(p[0]):null}}catch{return null}}async function z(e,a){var d,n,i,s;const t="https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=AIzaSyCAQYkzKXArqDV-_CIx0fQzLfLj5n0fmys",o={age:(d=e==null?void 0:e.demographics)==null?void 0:d.age,gender:(n=e==null?void 0:e.demographics)==null?void 0:n.gender,conditions:(e==null?void 0:e.conditions)||((i=e==null?void 0:e.health)==null?void 0:i.conditions)||[],medications:((e==null?void 0:e.medications)||[]).map(p=>p.name),healthFocus:e==null?void 0:e.healthFocus},c=a.map(p=>({id:p.id,label:p.label})),r={contents:[{parts:[{text:`
...
`}]}],generationConfig:{temperature:.2,response_mime_type:"application/json"}};try{const s=await u(o,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(l)});if(!s.ok)throw new Error("API Error");const p=await s.json();if((i=p.candidates)!=null&&i[0]){const y=p.candidates[0].content.parts[0].text.match(/\\[[\\s\\S]*\\]/);return y?JSON.parse(y[0]):null}}catch{return null}}async function D(e){var c;const a="https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=AIzaSyCAQYkzKXArqDV-_CIx0fQzLfLj5n0fmys",o={contents:[{parts:[{text:`
...
`}]}],generationConfig:{temperature:.3,response_mime_type:"application/json"}};try{const l=await u(a,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(o)});if(!l.ok)throw new Error("API Error");const r=await l.json();if((c=r.candidates)!=null&&c[0]){const n=r.candidates[0].content.parts[0].text.match(/\{[\s\S]*\}/);return n?JSON.parse(n[0]):null}}catch{return null}}async function B(e,a){var r;const t="https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=AIzaSyCAQYkzKXArqDV-_CIx0fQzLfLj5n0fmys",o=a.map(d=>d.name).join(", "),l={contents:[{parts:[{text:`
...
`}]}],generationConfig:{temperature:.1,response_mime_type:"application/json"}};try{const d=await u(t,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(l)});if(!d.ok)throw new Error("API Error");const n=await d.json();if((r=n.candidates)!=null&&r[0]){const s=n.candidates[0].content.parts[0].text.match(/\{[\s\S]*\}/);return s?JSON.parse(s[0]):null}}catch{return null}}async function G(e,a){var l,r,d,n;const t=a?`Patient Context: Age ${((l=a.personal)==null?void 0:l.age)||"unknown"}, Gender: ${((r=a.personal)==null?void 0:r.gender)||"unknown"}. Existing conditions: ${(((d=a.health)==null?void 0:d.conditions)||[]).join(", ")||"None"}.`:"",c={contents:[{parts:[{text:`You are an elite Clinical Pathway Simulator AI. 
...
}`}]}],generationConfig:{temperature:.2,response_mime_type:"application/json"}};try{const i=await u(f,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(c)});if(!i.ok)throw new Error("API Error");const s=await i.json();if((n=s.candidates)!=null&&n[0]){const m=s.candidates[0].content.parts[0].text.match(/\{[\s\S]*\}/);return m?JSON.parse(m[0]):null}}catch{return null}}async function W(e,a,t){var l,r,d,n;if(!e||e.length===0)return[];const c={contents:[{parts:[{text:`You are a clinical trials matching algorithm.
...
]`}]}],generationConfig:{temperature:.1,response_mime_type:"application/json"}};try{const i=await u(f,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(c)});if(!i.ok)throw new Error("API Error");const s=await i.json();if((n=s.candidates)!=null&&n[0]){const m=s.candidates[0].content.parts[0].text.match(/\[[\s\S]*\]/),y=m?JSON.parse(m[0]):[];return e.map(g=>{const h=y.find(w=>w.id===g.id)||{};return{...g,matchScore:h.matchScore||Math.floor(Math.random()*20)+60,aiContext:h.aiContext||"Relevance based on your active clinical hypotheses."}})}}catch{}return e}async function Q(e,a,t){var l,r,d,n;if(!e||e.length===0)return[];const c={contents:[{parts:[{text:`You are a clinical research AI.
