...
# 4. Replace the specific tab 4 content safely
guardrails_jsx = """
...
# Let's extract everything between {/* TAB 4: CLINICAL GUARDRAILS */} and </AnimatePresence> very safely.
start_idx = content.find("{/* TAB 4: CLINICAL GUARDRAILS */}")
end_idx = content.find("</AnimatePresence>", start_idx)

if start_idx != -1 and end_idx != -1:
    content = content[:start_idx] + guardrails_jsx + "\n        " + content[end_idx:]
else:
    print("Could not find boundaries")

with open(path, 'w', encoding='utf-8') as f:
    f.write(content)
print("Updated Dietician.tsx guardrails safely")
