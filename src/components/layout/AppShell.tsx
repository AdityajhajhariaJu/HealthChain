  ...
  return (
    <div className="app-shell">
      ...
      {!isMobile && (
        ...
        <motion.main className={`app-shell__content ${isMobile ? 'mobile' : ''}`} id="main-content" style={{ overflowY: isMobile && location.pathname.startsWith('/app/ava') ? 'hidden' : 'auto', paddingTop: undefined, paddingBottom: isMobile && location.pathname.startsWith('/app/ava') ? '0px' : undefined, transformOrigin: 'top center' }} onScroll={handleMainScroll} animate={{ scale: showMoreMenu || showProfileMenu ? 0.93 : 1, opacity: showMoreMenu || showProfileMenu ? 0.5 : 1, borderRadius: showMoreMenu || showProfileMenu ? '16px' : '0px' }} transition={{ scale: { type: 'spring', stiffness: 300, damping: 30 }, opacity: { duration: 0.2 }, borderRadius: { duration: 0.2 } }}>
