...
export const SensualLineChart = memo(() => {
  const [activeIndex, setActiveIndex] = useState<number | null>(null);

  // Fake data for the chart (7 days of "Health Score")
  const data = useMemo(() => [65, 78, 70, 85, 82, 95, 90], []);
  const max = 100;
  const width = 300;
  const height = 140;
  const padding = 20;
  ...
  const { points, pathData, areaData } = useMemo(() => {
    ...
    const path = pts.reduce((acc, point, i, a) => {
    ...
    return { points: pts, pathData: path, areaData: area };
  }, [data, max, width, height, padding]);
