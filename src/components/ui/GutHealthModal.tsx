  ...
  return createPortal(<>
    <FocusTrap isActive={!quickMealOpen}>
      <div role="dialog" aria-modal="true" aria-label="Gut Health" style={{ position: 'fixed', inset: 0, zIndex: 10000, background: 'rgba(15, 23, 42, 0.65)', backdropFilter: 'blur(6px)', display: 'grid', placeItems: 'center', padding: 10 }} onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
        <div style={{ width: 'min(100%,1080px)', height: 'min(94vh,980px)', background: '#FFFFFF', borderRadius: 24, border: '1px solid #F1E5E7', boxShadow: '0 24px 80px rgba(45, 25, 25, 0.2)', display: 'flex', flexDirection: 'column', overflow: 'hidden', color: '#0F172A' }}>
          ...
          {tab === 'studio' && <details style={{ padding: '7px clamp(12px,3vw,24px)', borderBottom: '1px solid #F1E5E7', background: '#FFFCFB', color: '#8D354B', fontSize: 12 }}><summary style={{ cursor: 'pointer', fontWeight: 700 }}>Records and visit notes</summary><nav aria-label="Gut Health sections" style={{ display: 'flex', gap: 6, overflowX: 'auto', paddingTop: 8 }}>
            {([['studio','Resolution Studio',Sparkles],['records','My records',CalendarDays],['visit','Visit notes',Clipboard]] as const).map(([id,label,Icon]) => <button key={id} type="button" aria-current={tab === id ? 'page' : undefined} onClick={() => { setTab(id); setMessage(''); }} style={{ ...button, minHeight: 36, background: tab === id ? '#FEF2F3' : '#FFFDFC', borderColor: tab === id ? '#F9D2D7' : '#F1E5E7', color: tab === id ? '#CD3153' : '#64748B', fontSize: 12.5, whiteSpace: 'nowrap', boxShadow: tab === id ? '0 2px 6px rgba(205, 49, 83, 0.12)' : 'none' }}><Icon size={14} />{label}</button>)}
          </nav></details>}
          {tab !== 'studio' && <nav aria-label="Gut Health sections" style={{ display: 'flex', padding: '8px clamp(12px,3vw,24px)', gap: 6, borderBottom: '1px solid #F1E5E7', overflowX: 'auto', background: '#FFFCFB' }}>
            {([['studio','Resolution Studio',Sparkles],['records','My records',CalendarDays],['visit','Visit notes',Clipboard]] as const).map(([id,label,Icon]) => <button key={id} type="button" aria-current={tab === id ? 'page' : undefined} onClick={() => { setTab(id); setMessage(''); }} style={{ ...button, minHeight: 36, background: tab === id ? '#FEF2F3' : '#FFFDFC', borderColor: tab === id ? '#F9D2D7' : '#F1E5E7', color: tab === id ? '#CD3153' : '#64748B', fontSize: 12.5, whiteSpace: 'nowrap' }}><Icon size={14} />{label}</button>)}
          </nav>}
