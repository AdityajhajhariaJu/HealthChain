...
export const JarvisCoreDark: React.FC<{ size?: number }> = ({ size = 200 }) => {
      ...
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} fill="none" xmlns="http://www.w3.org/2000/svg" style={{ display: 'block' }}>
        <defs>
          <radialGradient id={`jarvisGlow-${id}`} cx="50%" cy="50%" r="50%">
            ...
            <stop offset="70%" stopColor="#EA580C" stopOpacity="0.05" />
        ...
        <g className={`ring-outer-${id}`}>
          <circle cx={center} cy={center} r={maxR} stroke="#FFFFFF" strokeWidth="1" strokeDasharray="1 4" opacity="0.15" />
          {ticks.map((t, i) => (
            <line key={`tick-${i}`} x1={t.x1} y1={t.y1} x2={t.x2} y2={t.y2} stroke={t.isMajor ? '#F97316' : '#FFFFFF'} strokeWidth={t.isMajor ? 2 : 1} opacity={t.isMajor ? 0.9 : 0.2} />
          ))}
          <path d={`M ${center} ${center} L ${center + maxR} ${center} A ${maxR} ${maxR} 0 0 1 ${center + maxR * Math.cos(Math.PI/4)} ${center + maxR * Math.sin(Math.PI/4)} Z`} fill="#F97316" opacity="0.1" />
        </g>
        <g className={`ring-mid-${id}`}>
          <circle cx={center} cy={center} r={midR} stroke="#EA580C" strokeWidth="2" strokeDasharray="15 30 5 10 40 20" opacity="0.8" />
          <circle cx={center} cy={center} r={midR * 0.85} stroke="#FFFFFF" strokeWidth="1" strokeDasharray="4 4" opacity="0.2" />
        </g>
        <g className={`core-${id}`}>
          <circle cx={center} cy={center} r={coreR * 1.5} fill="#F97316" opacity="0.2" />
          <circle cx={center} cy={center} r={coreR} fill="#0F172A" stroke="#F97316" strokeWidth="2" />
          <circle cx={center} cy={center} r={coreR * 0.4} fill="#F97316" />
        </g>
      </svg>
    </div>
  );
