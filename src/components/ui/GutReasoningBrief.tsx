...
export const GutReasoningBrief: React.FC<Props> = ({ thread, evidence, synthesis, stale, topic, busy, error, onTopicChange, onGenerate, onRefine, onOpenSource, onAction, onMap }) => {
  ...
  return <section className={`gr-reasoning gr-answer-card ${ready ? 'gr-reasoning-ready' : 'gr-reasoning-pending'}`} aria-labelledby="gr-reasoning-title" aria-busy={busy}>
    ...
    {ready && !busy && <>
      <button type="button" className="gx-answer-map-link" onClick={onMap}><span className="gx-mini-map" aria-hidden="true"><i /><i /><i /></span><span><strong>See how this connects</strong><small>Your information · research · open questions</small></span><ArrowRight size={18} /></button>
      {!synthesis.followUpQuestion && <div className="gr-reason-next"><span><Check size={15} /></span><div><strong>What to do next</strong><p>{synthesis.nextReason}</p>{!synthesis.followUpQuestion && <button type="button" onClick={() => onAction(synthesis.nextAction)}>{actionLabels[synthesis.nextAction]} <ArrowRight size={15} /></button>}</div></div>}
