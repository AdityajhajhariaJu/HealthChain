...
export const GutCurrentConcern: React.FC<Props> = ({ thread, snapshot, busy, onsetDraft, onOnsetDraft, reflectionDraft, onReflectionDraft, onSaveOnset, onClearOnset, onSaveReflection, onCopyBrief, onOpenHistory, onOpenSource, onOpenResearch }) => {
  ...
  const sourceGuide = getGutPublicSourceGuide(thread.symptom);
  const careSource = sourceGuide?.url || 'https://www.niddk.nih.gov/health-information/digestive-diseases';
  ...
  return <section className="gr-current" aria-label="Current concern summary">
    ...
    <div className="gr-current-care"><ShieldCheck size={21} aria-hidden="true" /><div><strong>Severe, sudden, worsening, or otherwise concerning symptoms? Seek medical care promptly.</strong><p>This screen cannot assess urgency. {sourceGuide ? `${sourceGuide.title} is general information, not a finding about you.` : 'The NIDDK digestive health library offers general information.'} <a href={careSource} target="_blank" rel="noopener noreferrer">Read the NIDDK source <ArrowRight size={13} /></a></p></div></div>
