...
export const GutDecisionPlanner: React.FC<Props> = ({ thread, snapshot, observations, busy, onSave, onSymptomChange, onOpenQuickMeal }) => {
  ...
  const choose = async (key: 'a' | 'b') => {
    ...
    const next = { ...draft, chosen: key, chosenAt: new Date().toISOString(), outcome: null, outcomeAt: null };
  ...
  return <div className="gr-decision">
    ...
    <div className="gr-decision-actions"><button type="button" className="gr-secondary" disabled={busy || same(draft, saved)} onClick={() => void save(draft, 'Your options are saved. You can decide later.')}>Save options for later</button>{draft.chosen && <button type="button" className="gr-link" disabled={busy} onClick={() => void save({ ...draft, chosen: null, chosenAt: null, outcome: null, outcomeAt: null }, 'Choice cleared. Your options remain.')}>Leave undecided</button>}</div>
    {draft.chosen && <section className="gr-decision-return"><div><RotateCcw size={19} /><div><strong>When the situation has passed</strong><p>What did you actually choose or notice? You can skip this. A chosen option is not recorded as eaten.</p></div></div><label htmlFor="gr-decision-outcome">Your own outcome</label><textarea id="gr-decision-outcome" value={draft.outcome || ''} maxLength={1000} rows={3} onChange={(event) => setDraft({ ...draft, outcome: event.target.value })} placeholder="What happened, in your words?" /><div><button type="button" className="gr-primary" disabled={busy || (draft.outcome || '') === (saved.outcome || '')} onClick={() => void recordOutcome()}>Save what happened</button><button type="button" className="gr-link" onClick={onOpenQuickMeal}><Utensils size={16} /> Record an actual meal separately</button></div></section>}
