...
export const GutReviewedEvidencePanel: React.FC<{ symptom: GutSymptom; topic?: GutResearchTopic | null }> = ({ symptom, topic }) => {
  ...
  return <section className="gr-reviewed-evidence" aria-label="Source-backed general context and review status">
    ...
    {guide && <article className="gr-public-guide"><h5>{guide.title}</h5><p>{guide.sentence}</p><a href={guide.url} target="_blank" rel="noopener noreferrer">Read the NIDDK source · {guide.locator} <ArrowUpRight size={14} aria-hidden="true" /></a><details className="gr-public-guide-detail"><summary>How this source fits your question</summary><div><strong>For your question</strong><span>{guide.relevance}</span></div><div><strong>What it cannot tell us</strong><span>{guide.limit}</span></div><small>Public source: {guide.sourceReviewed}. HealthChain’s adaptation is pending independent clinical review.</small></details></article>}
    {topicGuide && <article className="gr-public-guide gr-topic-guide"><h5>{topicGuide.title}</h5><p>{topicGuide.sourceSays}</p><a href={topicGuide.url} target="_blank" rel="noopener noreferrer">Inspect the original · {topicGuide.locator} <ArrowUpRight size={14} aria-hidden="true" /></a><details className="gr-public-guide-detail"><summary>Who this source studied</summary><div><strong>Who or what was discussed</strong><span>{topicGuide.population}</span></div><div><strong>Fit for your question</strong><span>{topicGuide.fit}</span></div><small>Source-navigation note; no personal cause or treatment is inferred. Independent app review pending.</small></details></article>}
