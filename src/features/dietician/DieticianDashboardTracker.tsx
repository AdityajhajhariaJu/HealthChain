...
export function DieticianDashboardTracker({
  ...
  // Consolidated 6 separate array reductions into a single O(N) pass over the daily food logs.
  // This prevents iterating through the same array multiple times on every render.
  const { consumed, consumedProtein, consumedCarbs, consumedFats, consumedSugar, consumedFibre } =
    React.useMemo(() => {
      let c = 0,
        p = 0,
        ca = 0,
      ...
      const logs = foodLogs[currentDate];
      if (Array.isArray(logs)) {
        for (let i = 0; i < logs.length; i++) {
          ...
          p += log.protein || 0;
          ca += log.carbs || 0;
          f += log.fat || log.fats || 0;
          s += log.sugar || 0;
          fi += log.fibre || log.fiber || 0;
        }
      }
      return {
        consumed: c,
    ...
    return (
      ...
      >
        ...
        >
          ...
          >
            <span
              style={{ fontSize: '18px', fontWeight: 800, color: '#0F172A', lineHeight: '1.2' }}
            >
              {Math.round(value)}
            </span>
  ...
  const getConsumedForMeal = (mealName: string) => {
    ...
    return foodLogs[currentDate]
      .filter(
        (l: any) => l.type === mealName || (mealName === 'Morning Snack' && l.type === 'Snack')
      )
  ...
  return (
    ...
    >
          ...
          <div style={{ display: 'flex', gap: '14px', alignItems: 'center' }}>
            ...
            <div>
              ...
              <div style={{ fontSize: '16px', fontWeight: 800, color: '#0F172A' }}>
                {profile?.mealSchedule?.includes('16:8') ? '16:8 Protocol' : '14 hrs Active'}
              </div>
            </div>
          </div>
          <button
          ...
          <div style={{ display: 'flex', gap: '14px', alignItems: 'center' }}>
            ...
            <div>
              ...
              <div style={{ fontSize: '11px', color: '#94A3B8' }}>
                {waterGlasses * 250}ml of 2,000ml (Target: 8 glasses)
              </div>
      ...
      {/* Digestive Intelligence Quick Access Ribbon */}
      <div
        style={{
          display: 'grid',
          gridTemplateColumns: isMobile ? 'repeat(2, 1fr)' : 'repeat(4, 1fr)',
      ...
      >
        ...
        <button
          ...
          onClick={() => {
            triggerHapticLight();
            if (onSelectTab) onSelectTab('insights');
          }}
          style={{
            background: '#FFFFFF',
            padding: '12px 14px',
        ...
        <button
          ...
          onClick={() => {
            triggerHapticLight();
            if (onSelectTab) onSelectTab('calendar');
          }}
          style={{
            background: '#FFFFFF',
            padding: '12px 14px',
        ...
        >
          ...
          <div>
            ...
            <div style={{ fontSize: '11px', color: '#64748B' }}>Monthly Heatmap</div>
          </div>
        </button>
        ...
        <button
          ...
          onClick={() => {
            triggerHapticLight();
            if (onSelectTab) onSelectTab('sensitivities');
          }}
          style={{
            background: '#FFFFFF',
            padding: '12px 14px',
          ...
          return (
            <div key={idx}>
              ...
              >
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                  <span style={{ fontSize: '14px', color: '#64748B', fontWeight: 500 }}>
                    {mealConsumed} of {mealBudget} Cal
                  </span>
                  <button
                    type="button"
              ...
              {mealConsumed === 0 && (
                ...
                >
                  {meal.name === 'Breakfast' &&
                    'Hey, here are some Healthy Breakfast Suggestions for you'}
              ...
              {mealConsumed > 0 &&
                Array.isArray(foodLogs[currentDate]) &&
                foodLogs[currentDate]
                  .filter(
                    (l: any) =>
                      l.type === meal.name || (meal.name === 'Morning Snack' && l.type === 'Snack')
                  )
                  .map((log: any, idx2: number) => (
                    <div
                      key={idx2}
                      style={{
                        background: '#FFF',
      ...
      >
        ...
        <div>
          <strong style={{ color: '#0F172A' }}>Nutritional Estimates Basis:</strong> Calculated from
          standard household measures and Indian Food Composition Tables (IFCT/NIN). Cooking
          methods, ingredient variations, and digestive absorption vary. Not intended as laboratory
          calorimetry.
        </div>
      </div>
    </div>
