...
export function ImmersiveMediaCard({
  ...
  return (
    <motion.div
      ...
      className={elative overflow-hidden rounded-3xl cursor-pointer shadow-lg  flex-shrink-0}
      style={{
        backgroundImage: url(),
...
interface BottomSheetOverlayProps {
  ...
  onClose: () => void;
  children: React.ReactNode;
  bgImage?: string; // Optional hero image for the top half
}
      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed inset-x-0 bottom-0 z-[51] h-[92vh] flex flex-col bg-[#0F0F11] rounded-t-[32px] overflow-hidden shadow-2xl border-t border-white/10"
          >
            {/* Drag Handle Area */}
            <div 
      ...
      <SwimlaneCarousel title="Free Workouts @ Home">
        {workouts.map(workout => (
          <ImmersiveMediaCard
            ...
            aspectRatio="wide"
            tags={workout.tags}
            duration={${workout.duration} Min •  Cal}
