...
new_block = """      }}>
        ...
        <div style={{ padding: '0 24px 32px 24px' }}>
          <div onClick={() => { triggerHapticLight(); navigate('/app/war-room'); }} style={{ background: 'linear-gradient(135deg, #0F172A 0%, #1E293B 100%)', color: '#FFF', padding: '20px', borderRadius: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', cursor: 'pointer', boxShadow: '0 12px 32px rgba(15,23,42,0.2)' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
              <span style={{ fontSize: '18px', fontWeight: 700, letterSpacing: '-0.3px' }}>Collaborative Health Canvas</span>
...
with open(path, 'w', encoding='utf-8') as f:
    f.write(content)
print("Reorganized dashboard layout")
