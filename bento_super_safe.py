...
for i, line in enumerate(lines):
    if "return (" in line and "<div style={{ maxWidth: 1120" in lines[i+1]:
        start_idx = i
        break

if start_idx != -1:
    for i in range(start_idx, len(lines)):
        if "function EmptyAction" in lines[i]:
            end_idx = i
            break

if start_idx != -1 and end_idx != -1:
    # We found the block! Let's go up a few lines from EmptyAction to find the closing   );\n}
    actual_end_idx = end_idx
    while actual_end_idx > start_idx:
        if "  );\n" == lines[actual_end_idx-2] and "}\n" == lines[actual_end_idx-1]:
            actual_end_idx -= 2
            break
        actual_end_idx -= 1
        
    print(f"Start: {start_idx}, End: {actual_end_idx}")
    ...
    new_lines = lines[:start_idx] + [new_layout] + lines[actual_end_idx:]
    with open('src/features/dashboard/CaseDashboard.tsx', 'w', encoding='utf-8') as f:
        f.writelines(new_lines)
    print("Done safe replacement!")
