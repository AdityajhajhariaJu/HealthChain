...
# 2. Add Icons if needed
# Dietician already has Zap, Heart, ShieldCheck, Layers imported. Let's make sure it has Activity, Droplet, Brain, Flame
...
# 4. Add initialization from profile (if it exists)
...
# 5. Add handleGenerateGuardrails
  ...
  const handleGenerateGuardrails = async () => {
    ...
    try {
      ...
      if (data && data.guardrails) {
        ...
        awardPoints(2, 'Shield Activated', 'health', `guardrails_${Date.now()}`);
...
"""
# Replace the old static tab
content = re.sub(r"\{\/\* TAB 4: CLINICAL GUARDRAILS \*\/\}.*?\{\/\* TAB 4: CLINICAL GUARDRAILS \*\/\}\s*\{activeTab === 'guardrails' && \([\s\S]*?\}\s*\)\}\s*</motion\.div>\s*\)\}", guardrails_jsx, content, flags=re.DOTALL)
# The above regex might be tricky. Let's just split by `{/* TAB 4: CLINICAL GUARDRAILS */}`
parts = content.split("{/* TAB 4: CLINICAL GUARDRAILS */}")
if len(parts) >= 2:
    # Everything after TAB 4 until `</div>` that closes the tab content.
    # We can find `</AnimatePresence>` to know where the tab content ends
    sub_parts = parts[1].split("</AnimatePresence>")
    content = parts[0] + guardrails_jsx + "\n        </AnimatePresence>" + sub_parts[1]

with open(path, 'w', encoding='utf-8') as f:
    f.write(content)
print("Updated Guardrails in Dietician.tsx")
