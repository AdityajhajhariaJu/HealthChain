with open('restored.tsx', 'r', encoding='utf-8') as f:
    lines = f.readlines()
...
for i, line in enumerate(lines):
    if "return (" in line and "<div style={{ maxWidth: 1120, margin: '0 auto', paddingBottom: 40 }}>" in lines[i+1]:
        start_line = i
        break

if start_line != -1:
    for i in range(start_line, len(lines)):
        if "  );\n" == lines[i] and "}\n" == lines[i+1] and "function " in lines[i+3]:
            end_line = i
            break

print(f"Start: {start_line}, End: {end_line}")

new_layout_lines = '''  return (
        ...
        >
          <div>
            <h1 style={{ margin: 0, fontSize: isMobile ? 28 : 38, letterSpacing: -1.2, lineHeight: 1.1 }}>
              Good to see you
              {profile?.demographics?.name ? ', ' + (profile.demographics.name.split(' ')[0] or 'User') : ''}.
            </h1>
...
if start_line != -1 and end_line != -1:
    new_content = ''.join(lines[:start_line]) + new_layout_lines + ''.join(lines[end_line:])
