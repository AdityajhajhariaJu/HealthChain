...
if len(parts) == 2:
    ...
    # We want to replace from target down to the button area for "Medical Records"
    medical_label = "          <label style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontWeight: 800, color: '#0F172A', marginBottom: '12px' }}>\n            <span>Medical Records & Labs</span>"
    
    rest_parts = rest.split(medical_label)
    if len(rest_parts) == 2:
        bottom_chunk = rest_parts[1]
        ...
        with open(path, 'w', encoding='utf-8') as f:
            f.write(start_chunk + new_render + bottom_chunk)
        print("Success.")
    else:
        print("Medical label split failed.")
else:
    print("Target split failed.")
