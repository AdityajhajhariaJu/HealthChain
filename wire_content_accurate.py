...
content = content.replace(
    ...
    """<MeditationHeroCard item={{ title: 'A Diff Experience', cover_image_url: 'https://images.unsplash.com/photo-1434030216411-0b793f4b4173?w=800&q=80', type: 'meditation', id: 'm2' }} onClick={() => { triggerHapticLight(); setActiveMeditation({ id: 'm2', category_id: 'meditation', is_active: true, type: 'meditation', title: 'A Diff Experience', subtitle: 'Shift your perspective', description: 'A different kind of meditation. Uses subtle disruptions and shifts in audio to train your ability to refocus when distracted.', cover_image_url: 'https://images.unsplash.com/photo-1434030216411-0b793f4b4173?w=800&q=80', audio_url: '', video_url: '', duration_minutes: 20, calories_estimate: 0, difficulty: 'Advanced' }); }} getFallbackImage={getFallbackImage} />"""
)

# 2. Update Audio by Mood
# Inside the `.map((item, i) => (` block, `item` IS defined! So my regex replacement for Audio by Mood was perfectly fine, except for the unescaped quote `className="active-scale scroll-snap-item\"` which caused a syntax error.
audio_replace = """<div key={i} onClick={() => {
                    triggerHapticLight();
                    setActiveMeditation({
                      id: `mood-${i}`,
                      category_id: 'mood',
...
# wait, I shouldn't escape the quote `scroll-snap-item\"` because it becomes `\"` in typescript which is a syntax error!
audio_replace = audio_replace.replace('\\"', '"')
...
# Same here, `type` is defined inside the map `(type, i) =>`. So my regex for soundscapes was perfectly fine!
soundscape_replace = """onClick={() => {
                      triggerHapticLight();
                      setActiveMeditation({
                        id: `soundscape-${i}`,
